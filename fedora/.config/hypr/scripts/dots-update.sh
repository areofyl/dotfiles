...
if [ -z "$local_version" ]; then
  notify-send -i "$iDIR/error.png" "ERROR "!?!?!!"" "Unable to find dots version . exiting.... "
  exit 1
fi
...
if [ "$(echo -e "$github_version\n$local_version" | sort -V | head -n 1)" = "$github_version" ]; then
   notify-send -i "$iDIR/note.png" "Hyprland:" "No update available"
  exit 0
else
  # update available
  ...
  response=$($notify_cmd_shot "Hyprland:" "Update available! Update now?")
